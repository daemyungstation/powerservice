...
// Source File Name:   SEED_KISA.java

package powerservice.business.seed.kisa;
...
public class SEED_KISA
{
    ...
    private static void SeedRound(int L0[], int L1[], int R0[], int R1[], int K[])
    {
        ...
        int T1 = R1[0] ^ K[1];
        T1 ^= T0;
        T00 = T0 >= 0 ? T0 : (long)(T0 & 0x7fffffff) | 0xffffffff80000000L;
        T1 = SS0[GetB0(T1)] ^ SS1[GetB1(T1)] ^ SS2[GetB2(T1)] ^ SS3[GetB3(T1)];
        T11 = T1 >= 0 ? T1 : (long)(T1 & 0x7fffffff) | 0xffffffff80000000L;
        T00 += T11;
