  ...
  function fnLogin() { // 로그인
      ...
      var result = "";
      //if(top.centercd == "BILL") result = fnAgentWait("5");
      fnChangeAgentStatus(1);
      result = fnChangeAgentStatus(0);
      if(top.centercd == "BILL") result = fnChangeAgentStatus(2,3); //휴식으로 설정
...
//}
  ...
  // 메일상담후처리
  function fnMSetAfterCallWork(reasonCode) {
    var result = fnMChangeAgentStatus(5, reasonCode);
    return result;
  }
  ...
  function ueiSplit_Bill() {
      var callerData = CAPI3XEvt1.UEI;
      //var callerData = "100303631026902272919011gTZUi7D8FMfLqujt5A==0555520793      000";
      var customertype = "";	// 개인 "1", 사업자 "2"
      var customerid = "0000000000";		// 고객 아이디 기본값 미인증 고객
      var residentno = "";		// 등록번호 : 주민번호 또는 사업자 번호
      var residentnoEncode = "";// 등록번호 암호화
      var arscode = "";
      var ani_no = "";	// 발신번호
      var arsInfo = "";
      ...
      if(callerData.length > 63) {
          customertype = callerData.substr(0,1);
          customerid = callerData.substr(1,10);

          if(customertype == "1") {
              residentno = callerData.substr(11, 13);
              residentnoEncode = callerData.substr(24, 24);
              ani_no = callerData.substr(48, 16);
              arscode = callerData.substr(64, 3);
          } else if(customertype == "2") {
              residentno = callerData.substr(11, 10);
              residentnoEncode = callerData.substr(24, 24);
              ani_no = callerData.substr(48, 16);
              arscode = callerData.substr(64, 3);
          } else {
          ...
          arsInfo = getArsInfo(arscode);
      } else {
          // 내선일 경우
          ani_no = CAPI3XEvt1.OtherParty;
      }
      ...
      document.getElementById("anino").value = ani_no;
      document.getElementById("customerno").value = customerid;
      ...
      if(callerData.length > 70 || callerData.indexOf("|") != -1){
          // 호전환
          call_type = "2";
      }else if(callerData == ""){
          // 내선전화
      ...
      if (call_type == "3") {
          ...
          getBillCustomer(call_type, customerid, customertype, residentnoEncode);
      }
  }

  function getArsInfo(codecd) {
      codecd = "ARS" + codecd;
      for(var i=0 ; i < arsCode.length ; i++) {
          if(arsCode[i] == codecd) return arsValue[i];
      }
      return "";
  }
